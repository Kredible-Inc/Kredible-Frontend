...
import { User, UserRole, CreditScore, LendingTransaction, BorrowingTransaction } from "@/shared/types/user.types";
...
interface UserState {
  ...
  updateTransactionStatus: (transactionId: string, status: string, type: 'lending' | 'borrowing') => void;
  clearUserData: () => void;
  
  // Computed values
  getTotalLent: () => number;
  getTotalBorrowed: () => number;
  getActiveLoans: () => { lending: LendingTransaction[], borrowing: BorrowingTransaction[] };
...
export const useUserStore = create<UserState>()(
  persist(
    (set, get) => ({
      ...
      updateTransactionStatus: (transactionId: string, status: string, type: 'lending' | 'borrowing') => {
        set((state) => {
          if (type === 'lending') {
            return {
      ...
      getReputation: () => {
        const { lendingHistory, borrowingHistory } = get();
        const completedLending = lendingHistory.filter(t => t.status === 'completed').length;
        const completedBorrowing = borrowingHistory.filter(t => t.status === 'completed').length;
        const defaultedLending = lendingHistory.filter(t => t.status === 'defaulted').length;
        const defaultedBorrowing = borrowingHistory.filter(t => t.status === 'defaulted').length;
